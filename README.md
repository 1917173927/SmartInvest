...
# 首次建立 Chronos/基线校准样本（手动命令默认 20 日，需模型已缓存）
...
报告写入 `06-自动分析`，个股文件名为“日期-市场代码-股票名称-周期”，图表位于对应个股的 `charts/`。技术图包含 ATR 聚类支撑/压力带、K 线、MA、成交量、MACD 和 RSI；概率图单独展示 5/10/20/60/120 日中位路径、名义 80% 与近似 50% 分位区间。支撑/压力表另附 20 日 walk-forward 触达与守住统计，少于 20 次触达时标记为样本不足。行情、新闻、宏观、回执、校准与自动任务状态统一保存在 `.stock-analysis/analysis.sqlite3`。运行数据默认不提交 Git。
...
- 原始 OHLCV 与分红/拆股事件分开保存。A 股公司行动按除权日保存，将公开接口的每 10 股送转/派息换算为拆分比例和每股现金；同日多来源只采用一条高优先级事实，避免重复调整。行情和公司行动接口独立降级，行情主源成功但行动接口失败时会继续尝试备用行动源。回测不会直接采用可能随未来公司行动变化的动态前复权快照，仍无法解释的 35% 以上单日收益会明确告警。
...
支撑/压力强度是局部高低点触达、成交量、ATR 与时间衰减形成的启发式分数，不是守住概率，也不直接提高买入评分。报告中的历史守住率严格在每个截止点重新识别区间，仅统计之后真实触达的窗口，并给出 95% 区间。概率扇形图只在明确的预测期限上使用模型输出，中间交易日是可视化插值；实验期限未完成区间覆盖率校准时必须结合状态标签阅读。`stock auto` 默认轮询 5/10/20/60/120 日校准，但每次只执行一个重任务，其余记录为限额跳过，避免日常运行失控。
